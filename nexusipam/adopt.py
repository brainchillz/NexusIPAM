...
def target_pull(name):
    ...
    try:
        if kind == 'unifi':
            ...
            peer['verify'] = target.get('verify') or 'insecure'
            state = unifi.read_state(peer)
        elif kind in ('pihole', 'technitium'):
            return err('Adoption from a %s is not built yet — for now record '
                       'its state in the plan and push, which is the direction '
                       'a test instance wants anyway' % kind, 400)
